...
- 🆕 **Онбординг** — кнопка «?» показывает инструкцию для текущего экрана: на лендинге — о двух карточках, в редакторе — набор для текущей стороны (передняя или задняя). Каждый набор показывается автоматически при первом заходе на свою сторону.
...
│   ├── onboarding.js           # Три набора пошаговых инструкций: лендинг / передняя / задняя (overlay + spotlight)
...
- При первом открытии любой страницы (`index.html`, передняя сторона `back.html`, задняя сторона) показывается соответствующий набор инструкции из `js/onboarding.js`: лендинг — о двух карточках; передняя — про номер, тоглы, расширенные настройки, переключение сторон и отправку; задняя — про подпись, шрифты, логотипы, перемещение и кнопки. Каждый набор показывается только один раз — после закрытия (`localStorage["brelok-onboarding-{index|front|back}-seen"]`) не появится автоматически, но кнопка «?» открывает его в любой момент. Esc / тап мимо / «Пропустить» / «Готово» — закрывают.
