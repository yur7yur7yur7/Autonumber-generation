...
// One-shot migration: legacy `backLogoX`/`backLogoY` (single shared offset) →
// per-logo `backLogoPositions[logoFile]` Map. Returns true if migration happened.
function migrateLegacyLogoOffsets() {
    const oldX = settings.backLogoX;
    const oldY = settings.backLogoY;
    if (typeof oldX !== 'number' && typeof oldY !== 'number') return false;
    if (!settings.backLogoPositions || typeof settings.backLogoPositions !== 'object'
            || Array.isArray(settings.backLogoPositions)) {
        settings.backLogoPositions = {};
    }
    // Apply the legacy shared offset to every logo currently present in #customText.
    const customText = document.getElementById('customText');
    if (!customText) return false;
    const logos = customText.querySelectorAll('.inline-logo[data-logo-file]');
    const files = new Set();
    logos.forEach(el => files.add(el.dataset.logoFile));
    files.forEach(file => {
        if (!settings.backLogoPositions[file]) {
            settings.backLogoPositions[file] = { x: oldX || 0, y: oldY || 0 };
        }
    });
    delete settings.backLogoX;
    delete settings.backLogoY;
    return true;
}
...
function loadSettings() {
    ...
    try {
        ...
        if (saved) {
            ...
            migrated = migrateLegacyLogoOffsets();
        }
    } catch (e) {
        console.log('Не удалось загрузить настройки');
    }
    // Sanity-check: if margin is out of physical range (the slider's min/max), it
    // means a previous version or external tool saved a bogus value. Reset to default.
    if (typeof settings.margin !== 'number' || settings.margin < 0 || settings.margin > 100) {
        console.warn('[main] loadSettings: resetting invalid margin=', settings.margin,
                     'to DEFAULT_SETTINGS.margin=', DEFAULT_SETTINGS.margin);
    ...
    if (window.__overlayDebug === true) {
        console.log('[main] loadSettings: settings after load=', JSON.stringify({
            margin: settings.margin,
            backTextX: settings.backTextX,
            backTextY: settings.backTextY,
            backLogoPositions: settings.backLogoPositions,
            backFontSize: settings.backFontSize,
            backLogoSize: settings.backLogoSize,
            numberX: settings.numberX,
...
async function drawPlateImmediate() {
    ...
    if (window.__overlayDebug === true) {
        console.log('[main] drawPlateImmediate done, calling rebuildPreviewOverlay; side=', currentSide);
    }
